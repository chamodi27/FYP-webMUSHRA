...
function PairedComparisonPage(_reference, _condition, _pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler, _language) {
  ...
  this.preferenceABPlayback = {A: 0, B: 0};
  this.preferenceABUnlockThreshold = 0.95;
  
  this.time = 0;
  this.startTimeOnPage = null;
}

PairedComparisonPage.prototype.isPreferenceABMode = function () {
  return this.pageConfig.pairedComparisonMode === "preference_ab";
};

PairedComparisonPage.prototype.hasCompletedPreferenceABPlayback = function () {
  return this.preferenceABPlayback.A >= this.preferenceABUnlockThreshold
    && this.preferenceABPlayback.B >= this.preferenceABUnlockThreshold;
};

PairedComparisonPage.prototype.setPreferenceABChoiceEnabled = function (_enabled) {
  if (!this.isPreferenceABMode()) {
    return;
  }

  var radioButtons = $('#radio-choice input[type="radio"]');
  radioButtons.prop('disabled', !_enabled);

  if ($.fn.checkboxradio) {
    radioButtons.checkboxradio(_enabled ? 'enable' : 'disable');
  }
};

PairedComparisonPage.prototype.updatePreferenceABProgress = function (_event) {
  if (!this.isPreferenceABMode() || this.currentItem === null) {
    return;
  }
  ...
  var progress = Math.min(_event.currentSample / duration, 1);
  if (this.currentItem === "A") {
    this.preferenceABPlayback.A = Math.max(this.preferenceABPlayback.A, progress);
  } else if (this.currentItem === "B") {
    this.preferenceABPlayback.B = Math.max(this.preferenceABPlayback.B, progress);
  }

  if (this.hasCompletedPreferenceABPlayback()) {
    this.setPreferenceABChoiceEnabled(true);
  }
};
...
PairedComparisonPage.prototype.init = function () {
  ...
  this.mushraAudioControl.addEventListener((function (_event) {
    ...
    } else if (_event.name == 'processUpdate') {
      this.updatePreferenceABProgress(_event);
    }
  }).bind(this));
};
...
PairedComparisonPage.prototype.render = function (_parent) {
  ...
  var isPreferenceABMode = this.isPreferenceABMode();
  var trNames;
  if (isPreferenceABMode) {
    trNames = $("<tr><td>A</td><td>B</td></tr>");
  } else {
    trNames = $("<tr><td>" + this.pageManager.getLocalizer().getFragment(this.language, 'reference') + "</td><td>A</td><td>B</td></tr>");
  }
  ...
  var questionPrompt = this.pageConfig.questionText || this.pageManager.getLocalizer().getFragment(this.language, 'quest');
  var columnCount = isPreferenceABMode ? 2 : 3;
  var trQuestion = $("<tr><td colspan='" + columnCount + "'><br/><br/>" + questionPrompt + "</td></tr>");
  tableAB.append(trQuestion);


  var trResponse = $("<tr></tr>");
  tableAB.append(trResponse);
  var tdResponse = $("<td colspan='" + columnCount + "'></td>");
  trResponse.append(tdResponse);
  
  var optionALabel = this.pageConfig.optionALabel || "A";
  var optionBLabel = this.pageConfig.optionBLabel || "B";
  var optionAValue = isPreferenceABMode ? "prefer_a" : "a";
  var optionBValue = isPreferenceABMode ? "prefer_b" : "b";
  var disabledAttribute = isPreferenceABMode ? " disabled='disabled'" : "";
  var radioChoice = $("<div id='radio-choice' data-role='controlgroup' data-type='horizontal'>\
    <input type='radio' name='radio-choice' id='radio-choice-a' value='" + optionAValue + "'" + disabledAttribute + ">\
    <label for='radio-choice-a'>" + optionALabel + "</label>\
    <input type='radio' name='radio-choice' id='radio-choice-b' value='" + optionBValue + "'" + disabledAttribute + ">\
    <label for='radio-choice-b'>" + optionBLabel + "</label>\
  </div>");
...
PairedComparisonPage.prototype.btnCallbackB = function() {
  ...
  if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
    ...
    $(".audioControlElement").text(this.pageManager.getLocalizer().getFragment(this.language, 'playButton'));
    if (this.isPreferenceABMode()) {
      var referenceIndex = this.mushraAudioControl.getReferenceIndexOfConditions();
      var nonReferenceIndex = referenceIndex === 0 ? 1 : 0;
      this.mushraAudioControl.playCondition(nonReferenceIndex);
    } else {
...
PairedComparisonPage.prototype.load = function () {
  ...
  if (this.isPreferenceABMode()) {
    this.setPreferenceABChoiceEnabled(this.hasCompletedPreferenceABPlayback());
  }
  //choice 
  
  if(this.choice === 'a' || this.choice === 'prefer_a') {
  ...
  } else if (this.choice === 'b' || this.choice === 'prefer_b') {
...
PairedComparisonPage.prototype.store = function () {
  ...
  if (this.isPreferenceABMode()) {
    choice.reference = this.reference.getFilepath();
    choice.nonReference = this.condition.getFilepath();
    if (this.choice === null) {
